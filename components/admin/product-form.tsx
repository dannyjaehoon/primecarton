...
const ProductForm = ({type, product, productId} : {type:'Create'| 'Update', product?:Product; productId?: string}) => {
    ...
    const onSubmit: SubmitHandler<z.infer<typeof insertProductSchema>> = async (values) => {
        ...
        if(res.message === "") throw new Error("type is not matched");
                                ...
                                <CardContent className="space-y-2 mt-2 min-h-48">
                                    <div className="flex-start space-x-2">
                                        {images.map((image: string) => (
                                            <Image key={image} src={image} alt="product image" className="w-20 h-20 object-over object-center rounded-sm" width={100} height={100}/>  
                                        ))}
                                        <FormControl>
